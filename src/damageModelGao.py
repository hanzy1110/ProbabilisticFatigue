...
def inner(k, n_i:jnp.DeviceArray ,N_i:jnp.DeviceArray)->jnp.float32:
    ...
    if k == 0:
        exponent = jnp.true_divide(jnp.log(N_i[k+1]),jnp.log(N_i[k]))
        nNi = jnp.true_divide(n_i[k],N_i[k])
        return jnp.asarray((1-nNi)**(exponent))

    else:
        exponent = jnp.true_divide(jnp.log(N_i[k+1]),jnp.log(N_i[k]))
        nNi = jnp.true_divide(n_i[k],N_i[k])
        return jnp.asarray((inner(k-1, n_i, N_i)-nNi)**(exponent))
...
def gaoModel(n_i:jnp.DeviceArray, N_i:jnp.DeviceArray)->jnp.DeviceArray:
    ...
    print('outer nNi', nNi)
    return jnp.asarray(-1/(jnp.log(N_i[-1])) * (inner(totalN-1, n_i, N_i)-nNi))
...
def gaoModel_debug(n_i:jnp.DeviceArray, N_i:jnp.DeviceArray, lnN_i)->jnp.DeviceArray:
    ...
    nNi = n_i[-1]/N_i[-1]
    print('outer nNi', nNi)
    return np.asarray(-1/(lnN_i[-1]) * (inner_debug(totalN-1, n_i, N_i, lnN_i)-nNi))
