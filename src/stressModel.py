...
class CableProps:
    ...
    def PFSlopeEval(self, Eal, EIMin, T, a:float=89.0):
        ...
        return Eal*(2*self.rw) * lambda_**2/(4*(np.exp(-lambda_*a)-1+lambda_*a))
...
class PFSlopeModel:
    ...
    def build_model(self):
        with pm.Model() as self.model:
            ...
            pfSlope = pm.Deterministic('pfSlope', self.cable.PFSlopeEval(Eal, 1e8*EIMin, T))
            noise = pm.Gamma('sigma', alpha=1, beta=2)
            likelihood = pm.Normal('likelihood', mu=pfSlope, sigma=noise, observed=self.ydata)
