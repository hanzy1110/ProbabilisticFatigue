...
def StudentTSim(sigma_f, mu_f , size=(len(S),)):
    return stats.norm(loc=mu_f, scale=sigma_f).rvs()
...
with pm.Model() as SNCurveModel:
    ...
    sigma_f_ = σ_gp.prior("sigma_f_", X=S)
    sigma_f = pm.Deterministic('sigma_f', pm.math.abs_(sigma_f_))
    ...
    sim = pm.Simulator('likelihood', StudentTSim,
                       sum_stat='sort', epsilon=1,
                       params=(sigma_f, mu_f), observed=log_N)

    # trace = pm.sample(target_accept=0.95, chains=2, return_inferencedata=True, random_seed=2022)
    trace = pm.sample_smc(draws=2000, parallel=True,kernel='ABC' )
